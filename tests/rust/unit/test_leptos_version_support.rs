...
fn test_leptos_version_detection() {
    ...
    assert!(version.is_some(), "Should be able to detect Leptos version");
    
    let version = version.unwrap();
    assert!(version >= "0.8.8", "Should support Leptos 0.8.8 or higher");
}
...
fn test_leptos_feature_detection() {
    ...
    assert!(features.contains("basic_state_management"));
    assert!(features.contains("state_machines"));
    assert!(features.contains("stores"));
    
    // These features might be version-dependent
    if leptos_state::leptos_version().unwrap() >= "0.8.8" {
        assert!(features.contains("advanced_hooks"));
        assert!(features.contains("performance_optimizations"));
    }
}
...
fn test_version_specific_machine_creation() {
    ...
    assert!(machine.is_initialized(), "Machine should be properly initialized");
}
...
fn test_migration_guidance() {
    ...
    assert!(!guidance.is_empty(), "Migration guidance should not be empty");
    assert!(guidance.contains("0.8.8"), "Guidance should mention target version");
}
...
fn test_api_stability() {
    ...
    let machine = leptos_state::Machine::new();
    assert!(machine.is_ok(), "Core API should remain stable");
    
    let store = leptos_state::Store::new();
    assert!(store.is_ok(), "Store API should remain stable");
}
...
fn test_version_constraints() {
    ...
    assert!(constraints.minimum_version >= "0.8.8");
    assert!(constraints.recommended_version >= "0.8.8");
    assert!(constraints.maximum_version.is_none() || constraints.maximum_version >= Some("0.8.8"));
}
...
fn test_feature_flags() {
    ...
    assert!(flags.contains("leptos-0-8-8"));
    ...
    if current_version >= "0.9.0" {
        assert!(flags.contains("leptos-0-9-0"));
    }
}
...
fn test_compatibility_matrix() {
    ...
    for version in &["0.8.8", "0.8.9", "0.9.0"] {
        if matrix.contains_key(*version) {
            let compatibility = &matrix[*version];
            assert!(compatibility.supported, "Supported versions should be marked as compatible");
        }
    }
}
...
fn test_upgrade_recommendations() {
    ...
    assert!(recommendations.target_version >= "0.8.8", "Should recommend supported version");
...
fn test_version_specific_error_messages() {
    ...
    assert!(error.to_string().contains("0.8.8"), "Error should mention minimum version");
    assert!(error.to_string().contains("upgrade"), "Error should suggest upgrade");
    assert!(error.to_string().contains("leptos"), "Error should mention Leptos");
}
...
fn test_version_specific_examples() {
    ...
    for example in examples {
        assert!(example.code.contains("leptos_state"), "Example should use leptos_state");
...
fn test_feature_compatibility_validation() {
    ...
    let result = leptos_state::validate_feature_compatibility("experimental_feature");
...
fn test_version_specific_performance() {
    ...
    assert!(performance.initialization_time > 0, "Should report initialization time");
}
