...
pub trait Middleware<S: Store> {
    ...
    fn wrap(
        &self,
        next: Box<dyn Fn(&S::State) -> S::State>,
...
impl<S: Store> MiddlewareChain<S> {
    ...
    pub fn apply(&self, base: impl Fn(&S::State) -> S::State + 'static) -> impl Fn(&S::State) -> S::State {
...
mod tests {
    ...
    fn middleware_chain_applies_in_reverse_order() {
        ...
        let wrapped = chain.apply(|state| TestState { count: state.count + 1 });
    ...
    fn validation_middleware_prevents_invalid_states() {
        let validator: ValidationMiddleware<TestStore, _> = ValidationMiddleware::new(|state: &TestState| state.count >= 0);
