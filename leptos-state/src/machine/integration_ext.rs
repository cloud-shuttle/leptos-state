...
>
{
    ...
    fn send_integration_event(
        &self,
        manager: &IntegrationManager<C, E>,
        event: IntegrationEvent,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Result<(), IntegrationError>> + Send + '_>,
    >;

    /// Create a WebSocket integration
    fn with_websocket_integration(
        &self,
        manager: &mut IntegrationManager<C, E>,
        config: ConnectionConfig,
    ) -> Result<(), String>;

    /// Create an HTTP API integration
    fn with_http_integration(
        &self,
        manager: &mut IntegrationManager<C, E>,
        config: ConnectionConfig,
    ) -> Result<(), String>;
    ...
    IntegrationBuilder<C, E>
{
    ...
    pub fn with_websocket(mut self, config: ConnectionConfig) -> Self {
        let adapter = WebSocketAdapter::new(config);
        self.manager
            .register_adapter("websocket".to_string(), Box::new(adapter));
        self
    }
    ...
    pub fn with_http_api(mut self, config: ConnectionConfig) -> Self {
        let adapter = HttpApiAdapter::new(config);
        self.manager
            .register_adapter("http_api".to_string(), Box::new(adapter));
        self
    }
    ...
    pub fn with_database(mut self, config: ConnectionConfig) -> Self {
        let adapter = DatabaseAdapter::new(config);
        self.manager
            .register_adapter("database".to_string(), Box::new(adapter));
        self
    }
    ...
    pub fn with_message_queue(mut self, config: ConnectionConfig) -> Self {
        ...
        self.manager
            .register_adapter("message_queue".to_string(), Box::new(adapter));
        self
    }
    ...
    pub fn with_file_system(
        ...
        self.manager
            .register_adapter("filesystem".to_string(), Box::new(adapter));
        self
    }
