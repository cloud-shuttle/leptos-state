...
"#
        ...
        for state_name in machine.get_states() {
            if let Some(state_node) = machine.states_map().get(state_name) {
                for transition in &state_node.transitions {
                    ...
                    transition_code =
                        transition_code.replace("{transition_name}", &transition_name);
        ...
        for state_name in machine.get_states() {
            if let Some(state_node) = machine.states_map().get(state_name) {
                for (i, transition) in state_node.transitions.iter().enumerate() {
                    let guard_name =
                        format!("guard_{}_transition_{}", state_name.to_lowercase(), i);
    ...
    fn generate_tests(
        ...
        match self.config.language {
            ...
            ProgrammingLanguage::TypeScript => {
                self.generate_typescript_tests(machine, machine_name)
            }
    ...
    fn generate_rust_tests(
        ...
        result.push_str(&format!(
            "        let machine = {}::new(Default::default());\n",
            machine_name
        ));
        result.push_str(&format!(
        ...
        result.push_str(&format!(
            "        let mut machine = {}::new(Default::default());\n",
            machine_name
        ));
    ...
    fn generate_typescript_tests(
        ...
        result.push_str(&format!(
            "        const machine = new {}(defaultContext);\n",
            machine_name
        ));
        result.push_str(&format!(
    ...
    fn generate_python_tests(
        ...
        result.push_str(&format!(
            "        machine = {}(default_context)\n",
            machine_name
        ));
        result.push_str(&format!(
            "        self.assertEqual(machine.get_current_state(), '{}')\n",
            machine.initial_state()
        ));
        result.push('\n');
