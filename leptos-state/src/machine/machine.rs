...
// Extension traits
#[cfg(feature = "persist")]
use crate::machine::persistence::{MachinePersistenceExt, PersistentMachine, PersistenceConfig};
#[cfg(feature = "visualization")]
use crate::machine::visualization::{MachineVisualizationExt, VisualizedMachine, VisualizationConfig, ExportFormat};
#[cfg(feature = "testing")]
use crate::machine::testing::{MachineTestingExt, MachineTestRunner, TestConfig, DataStrategy};
#[cfg(feature = "performance")]
use crate::machine::performance::{MachinePerformanceExt, OptimizedMachine, PerformanceConfig, OptimizationStrategy};
#[cfg(feature = "integration")]
use crate::machine::integration::{IntegrationManager, IntegrationConfig, EventRoutingConfig, ErrorHandlingStrategy, RetryConfig};
#[cfg(feature = "documentation")]
use crate::machine::documentation::{DocumentationGenerator, DocumentationConfig, DocumentationFormat, DocumentationTemplate, DocumentationStyling, MachineDocumentationExt};
#[cfg(feature = "codegen")]
use crate::machine::codegen::{CodeGenerator, CodeGenConfig, ProgrammingLanguage};
...
impl<C: Clone + Send + Sync + 'static, E: Clone + 'static> MachineBuilder<C, E> {
    ...
    pub fn build_with_visualization(
        self, 
    ...
    #[cfg(feature = "codegen")]
    pub fn build_with_code_generation(
        self,
...
impl<C: Clone + Send + Sync + 'static, E: Clone + 'static> StateBuilder<C, E> {
    ...
    where
        F: Fn(&mut C, &E) + 'static,
    {
        self.entry_actions.push(Box::new(actions::FunctionAction::new(func)));
        self
    }
    ...
    where
        F: Fn(&mut C, &E) + 'static,
    {
        self.exit_actions.push(Box::new(actions::FunctionAction::new(func)));
        self
    }
    ...
    where
        ...
        E: std::fmt::Debug,
    {
        self.entry_actions.push(Box::new(actions::LogAction::new(message)));
        self
    }
    ...
    where
        ...
        E: std::fmt::Debug,
    {
        self.exit_actions.push(Box::new(actions::LogAction::new(message)));
        self
    }
    ...
    where
        F: Fn() + 'static,
    {
        self.entry_actions.push(Box::new(actions::PureAction::new(func)));
        self
    }
    ...
    where
        F: Fn() + 'static,
    {
        self.exit_actions.push(Box::new(actions::PureAction::new(func)));
        self
    }
    ...
    pub fn state(mut self, id: &str) -> StateBuilder<C, E> {
        ...
        self.machine_builder.states.insert(self.current_state, state_node);
...
impl<C: Clone + 'static + Send + Sync, E: Clone + 'static> ChildTransitionBuilder<C, E> {
    ...
    where
        ...
        T: PartialEq + 'static,
    {
        self.guards.push(Box::new(guards::FieldEqualityGuard::new(field_extractor, expected_value)));
        self
    }
    ...
    where
        ...
        T: PartialOrd + 'static,
    {
        self.guards.push(Box::new(guards::RangeGuard::new(field_extractor, min, max)));
        self
    }
    ...
    pub fn guard_max_transitions(mut self, max_count: usize) -> Self {
        self.guards.push(Box::new(guards::CounterGuard::new(max_count)));
        self
    }
...
impl<C: Send + Sync + Clone, E: Clone> Machine<C, E> {
    ...
    fn transition_simple(&self, state: &MachineStateImpl<C>, state_id: &str, event: E) -> MachineStateImpl<C>
    where
        E: PartialEq,
    {
        if let Some(state_node) = self.states.get(state_id) {
            ...
            for transition in &state_node.transitions {
                if transition.event == event {
                    // Check all guards
                    let guards_pass = transition.guards.iter()
...
mod tests {
    ...
    fn machine_transitions() {
        ...
        assert_eq!(running_state.value(), &StateValue::Simple("running".to_string()));
        
        let back_to_idle = machine.transition(&running_state, TestEvent::Stop);
        assert_eq!(back_to_idle.value(), &StateValue::Simple("idle".to_string()));
    }
